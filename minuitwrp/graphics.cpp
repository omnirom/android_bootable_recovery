...
{
    return (TW_HWROTATION == 0 || TW_HWROTATION == 180) ?
           (x < 0 || x >= gr_draw->width || y < 0 || y >= gr_draw->height) :
           (y < 0 || y >= gr_draw->width || x < 0 || x >= gr_draw->height);
}
...
void gr_clip(int x, int y, int w, int h)
{
    ...
    x0_disp = HWROTATION_X_DISP(x, y, gr_draw);
    y0_disp = HWROTATION_Y_DISP(x, y, gr_draw);
    x1_disp = HWROTATION_X_DISP(x + w, y + h, gr_draw);
    y1_disp = HWROTATION_Y_DISP(x + w, y + h, gr_draw);
    l_disp = min(x0_disp, x1_disp);
    r_disp = max(x0_disp, x1_disp);
    t_disp = min(y0_disp, y1_disp);
    b_disp = max(y0_disp, y1_disp);
...
void gr_line(int x0, int y0, int x1, int y1, int width)
{
    ...
    x0_disp = HWROTATION_X_DISP(x0, y0, gr_draw);
    y0_disp = HWROTATION_Y_DISP(x0, y0, gr_draw);
    x1_disp = HWROTATION_X_DISP(x1, y1, gr_draw);
    y1_disp = HWROTATION_Y_DISP(x1, y1, gr_draw);
...
void gr_blit(gr_surface source, int sx, int sy, int w, int h, int dx, int dy)
{
    ...
    // Figuring out display coordinates works for TW_HWROTATION == 0 too,
    // and isn't as expensive as allocating and rotating another surface,
    // so we do this anyway.
    dx0_disp = HWROTATION_X_DISP(dx, dy, gr_draw);
    dy0_disp = HWROTATION_Y_DISP(dx, dy, gr_draw);
    dx1_disp = HWROTATION_X_DISP(dx + w, dy + h, gr_draw);
    dy1_disp = HWROTATION_Y_DISP(dx + w, dy + h, gr_draw);
    l_disp = min(dx0_disp, dx1_disp);
    r_disp = max(dx0_disp, dx1_disp);
    t_disp = min(dy0_disp, dy1_disp);
    b_disp = max(dy0_disp, dy1_disp);
    ...
    // Skip the **(TW_HWROTATION == 0)** || (TW_HWROTATION == 180) check
    // because we are under a TW_HWROTATION != 0 conditional compilation statement
    surface_rotated.width   = (TW_HWROTATION == 180) ? surface->width  : surface->height;
    surface_rotated.height  = (TW_HWROTATION == 180) ? surface->height : surface->width;
    ...
    surface_hwrotation_transform((gr_surface) &surface_rotated, (const gr_surface) surface, 4);
