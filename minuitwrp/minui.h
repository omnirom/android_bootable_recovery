...
#define HWROTATION_X_DISP(x, y, surface) \
    ((TW_HWROTATION ==   0) ? (x) : \
     (TW_HWROTATION ==  90) ? ((surface)->width - (y) - 1) : \
     (TW_HWROTATION == 180) ? ((surface)->width - (x) - 1) : \
     (TW_HWROTATION == 270) ? (y) : -1)
#define HWROTATION_Y_DISP(x, y, surface) \
    ((TW_HWROTATION ==   0) ? (y) : \
     (TW_HWROTATION ==  90) ? (x) : \
     (TW_HWROTATION == 180) ? ((surface)->height - (y) - 1) : \
     (TW_HWROTATION == 270) ? ((surface)->height - (x) - 1) : -1)

void surface_hwrotation_transform(gr_surface dst_ptr, const gr_surface src_ptr, size_t num_bytes_per_pixel);
