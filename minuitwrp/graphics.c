...
int gr_init(void)
{
    ...
    gr_mem_surface = *((GGLSurface*) init_display_surface(vi.xres, vi.yres, vi.xres_virtual, PIXEL_FORMAT));
...
{
    GGLSurface* ms = (GGLSurface*) init_display_surface(vi.xres, vi.yres, vi.xres_virtual, PIXEL_FORMAT);
    if (!ms)    return -1;
