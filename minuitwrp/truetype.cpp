                    ...
                    const gr_surface gr_draw_surface)
{
    ...
    string_surface_rotated.width   = (TW_HWROTATION == 180) ? e->surface.width  : e->surface.height;
    string_surface_rotated.height  = (TW_HWROTATION == 180) ? e->surface.height : e->surface.width;
    ...
    surface_hwrotation_transform((gr_surface) &string_surface_rotated, (const gr_surface) &e->surface, 1);
#endif
    ...
    x1_disp = HWROTATION_X_DISP(x + e->surface.width, y_bottom, gr_draw);
    y1_disp = HWROTATION_Y_DISP(x + e->surface.width, y_bottom, gr_draw);
