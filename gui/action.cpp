...
int GUIAction::repackimage(std::string arg)
{
	int op_status = 1;
	operation_start("Repack Image");
	if (!simulate)
	{
		if (!TWFunc::Path_Exists("/sbin/magiskboottwrp")) {
			LOGERR("Image repacking tool not present in this TWRP build!");
			goto exit;
		}
		DataManager::SetProgress(0);
		TWPartition* part = PartitionManager.Find_Partition_By_Path(arg);
		if (part)
			gui_msg(Msg("unpacking_image=Unpacking {1}...")(part->Display_Name));
		else {
			gui_msg(Msg(msg::kError, "unable_to_locate=Unable to locate {1}.")(arg));
			goto exit;
		}
		if (!PartitionManager.Prepare_Repack(part, REPACK_ORIG_DIR, DataManager::GetIntValue("tw_repack_backup_first") != 0, "Repack Boot"))
			goto exit;
		DataManager::SetProgress(.25);
		gui_msg(Msg("unpacking_image=Unpacking {1}...")(DataManager::GetStrValue("tw_filename")));
		if (!PartitionManager.Prepare_Repack(DataManager::GetStrValue("tw_filename"), REPACK_NEW_DIR, true))
			goto exit;
		DataManager::SetProgress(.5);
		gui_msg(Msg("repacking_image=Repacking {1}...")(part->Display_Name));
		std::string command;
		if (DataManager::GetIntValue("tw_repack_kernel") != 0) {
			// Repack the kernel
			if (TWFunc::copy_file(REPACK_NEW_DIR "kernel", REPACK_ORIG_DIR "kernel", 0644)) {
				LOGERR("Failed to copy kernel\n");
				goto exit;
			}
			if (TWFunc::Path_Exists(REPACK_NEW_DIR "dtb")) {
				if (TWFunc::copy_file(REPACK_NEW_DIR "dtb", REPACK_ORIG_DIR "dtb", 0644)) {
					LOGERR("Failed to copy dtb\n");
					goto exit;
				}
			}
		} else {
		...
		command = "cd " REPACK_ORIG_DIR " && /sbin/magiskboottwrp --repack " REPACK_ORIG_DIR "boot.img";
...
int GUIAction::fixabrecoverybootloop(std::string arg __unused)
{
	...
	if (!simulate)
	{
		...
		std::string command = "cd " REPACK_ORIG_DIR " && /sbin/magiskboottwrp --hexpatch kernel 77616E745F696E697472616D667300 736B69705F696E697472616D667300";
		if (TWFunc::Exec_Cmd(command) != 0) {
