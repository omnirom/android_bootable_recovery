...
ActionThread::ActionThread()
{
	...
	m_funcMap["flashimage"] = &GUIAction::flashimage;
}
...
void ActionThread::threadAction(GUIAction *act, const std::string& func, const std::string& arg, int isThreaded)
{
	if (isThreaded) {
...
int GUIAction::doActions()
{
	...
	if (mActions.size() > 1) {
		for (it = mActions.begin(); it != mActions.end(); ++it) {
			if (checkIfActionNeedsThreading(*it)) {
				isThreaded = 1;
				break;
			}
		}
	...
	if (isThreaded) {
		// For multi-action, we create a thread to run all actions if any of the actions require a thread and run all actions in order in the same thread
		pthread_t t;

		int ret = pthread_create(&t, NULL, thread_start, this);
		if (ret) {
...
int GUIAction::checkIfActionNeedsThreading(Action action)
{
...
int GUIAction::doAction(Action action, int isThreaded /* = 0 */)
{
	...
	action_thread.threadAction(this, action.mFunction, action.mArg, isThreaded);
	return 0;
}
