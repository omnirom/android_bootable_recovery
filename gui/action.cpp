	...
	: GUIObject(node)
{
	...
	mf["installhtcdumlock"] = &GUIAction::partitionsd;
	...
	mf["decrypt"] = &GUIAction::reinjecttwrp;
...
int GUIAction::queuezip(std::string arg) {
	int zip_queue_index;
	string fn;
	DataManager::GetValue(TW_ZIP_QUEUE_COUNT, zip_queue_index);
	zip_queue_index++;
	...
	DataManager::GetValue("tw_filename", fn);
	if (strlen(fn.c_str()) > 0) {
		DataManager::SetValue(TW_ZIP_QUEUE_COUNT, zip_queue_index);
		DataManager::SetVectorValue("tw_filename_queue", fn);
	}
	return 0;
}
...
int GUIAction::queueclear(std::string arg) {
	int zip_queue_index;
	vector <string> tw_fn_queue; 

	DataManager::GetValue(TW_ZIP_QUEUE_COUNT, zip_queue_index);
	zip_queue_index = 0;
	DataManager::SetValue(TW_ZIP_QUEUE_COUNT, zip_queue_index);
	DataManager::GetVectorValue("tw_filename_queue", tw_fn_queue);
	tw_fn_queue.clear();
	return 0;
}
...
int GUIAction::flash(std::string arg) {
	...
	for (i = 0; i < zip_queue_index; i++) {
		...
		//DataManager::SetValue("tw_filename", DataManager::GetVecValue(zip_queue[i]);
		DataManager::SetValue(TW_ZIP_INDEX, (i + 1));

		TWFunc::SetPerformanceMode(true);
		ret_val = flash_zip(tw_fn_queue[i], arg, simulate, &wipe_cache);
