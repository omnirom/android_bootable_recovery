...
int GUIAction::flash(std::string arg)
{
	...
	/*if (DataManager::GetIntValue("tw_install_reboot") > 0 && ret_val == 0) {
		int reboot_seconds = DataManager::GetIntValue("tw_sleep");
		gui_msg(Msg(msg::kHighlight, "install_reboot=Rebooting in {1} second(s)")(reboot_seconds));
		usleep(1000000 * reboot_seconds);
