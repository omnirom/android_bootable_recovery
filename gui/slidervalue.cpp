...
GUISliderValue::GUISliderValue(xml_node<>* node) : GUIObject(node)
{
	...
	if (child)
	{
		mLineH = LoadAttrIntScaleY(child, "lineh");
		mLinePadding = LoadAttrIntScaleX(child, "linepadding");
		mSliderW = LoadAttrIntScaleX(child, "sliderw");
		mSliderH = LoadAttrIntScaleY(child, "sliderh");
	}
