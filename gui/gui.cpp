		...
		if (DataManager::GetIntValue("tw_busy") != 0) {
			...
			if (gui_console_only() == 0) {
				...
				if (strlen(command) > 11 && strncmp(command, "runscript", 9) == 0) {
					...
					char* pass = command + 8;
					gui_print("Attempting to decrypt data via command line.\n");
...
static int runPages(const char *page_name, const int allow_commands, const int stop_on_page_done)
{
	if (page_name)
...
#endif
	return runPages(NULL, 1, 0);
}

extern "C" int gui_startPage(const char *page_name, const int allow_commands)
{
	if (!gGuiInitialized)
		return -1;
...
#ifndef TW_OEM_BUILD
	if (!gCmdLineRunning && allow_commands)
	{
		setup_ors_command();
		gCmdLineRunning = 1;
	}
	...
	return runPages(page_name, allow_commands, 1);
}
