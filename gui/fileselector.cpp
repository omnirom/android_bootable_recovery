	...
	if (updateFileList) {
		...
		if (GetFileList(value) == 0) {
			...
			for (int index = 0; index < listSize; index++) {
				...
				if (index < folderSize) {
					data.displayName = mFolderList.at(index).fileName;
					data.displayResource = mFolderIcon;
				} else {
					data.displayName = mFileList.at(index - folderSize).fileName;
					data.displayResource = mFileIcon;
				}
...
int GUIFileSelector::Update(void)
{
	...
	if (mUpdate || updateFileList || scrollList->mUpdate) {
		mUpdate = 0;
		if (Render() == 0)
			return 2;
	}
...
int GUIFileSelector::NotifyTouch(TOUCH_STATE state, int x, int y)
{
	...
	if (state == TOUCH_RELEASE && scrollList->itemChosen >= 0) {
		...
		if (scrollList->itemChosen < folderSize + fileSize) {
			if (scrollList->itemChosen < folderSize) {
				std::string cwd;

				str = mFolderList.at(scrollList->itemChosen).fileName;
				if (mSelection != "0")
				...
				str = mFileList.at(scrollList->itemChosen - folderSize).fileName;
				if (mSelection != "0")
					DataManager::SetValue(mSelection, str);
