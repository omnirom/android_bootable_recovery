...
int GUIFileSelector::GetListSize(void)
{
...
int GUIFileSelector::GetListItem(int item_index, std::string &text)
{
	int folderSize = mShowFolders ? mFolderList.size() : 0;
	int fileSize = mShowFiles ? mFileList.size() : 0;

	if (item_index < 0 || item_index >= folderSize + fileSize) {
		icon = NULL;
		text = "";
		return -1;
	} else {
...
void GUIFileSelector::NotifySelect(int item_selected)
{
	int folderSize = mShowFolders ? mFolderList.size() : 0;
	int fileSize = mShowFiles ? mFileList.size() : 0;

	if (item_selected >= 0 && item_selected < folderSize + fileSize) {
		...
		LOGINFO("Invalid item selected %i in GUIListBox::NotifySelect\n", item_selected);
	mUpdate = 1;
}
