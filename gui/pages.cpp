...
int PageSet::SetOverlay(Page* page)
{
	...
	if (mOverlayPage)
	{
		...
		mOverlays.push_back(mOverlayPage);
	} else {
		if (mOverlays.size()) {
			mOverlays.pop_back();
			if (mOverlays.size())
...
{
	std::vector<Page*>::reverse_iterator iter;

	for (iter = mOverlays.rbegin(); iter != mOverlays.rend(); iter++) {
		if ((*iter))
			return ((*iter)->NotifyTouch(state, x, y));
	}
...
int PageSet::NotifyKey(int key, bool down)
{
	...
	for (iter = mOverlays.rbegin(); iter != mOverlays.rend(); iter++) {
		if ((*iter))
			return ((*iter)->NotifyKey(key, down));
	}
...
int PageSet::NotifyKeyboard(int key)
{
	...
	for (iter = mOverlays.rbegin(); iter != mOverlays.rend(); iter++) {
		if ((*iter))
			return ((*iter)->NotifyKeyboard(key));
	}
...
int PageSet::SetKeyBoardFocus(int inFocus)
{
	...
	for (iter = mOverlays.rbegin(); iter != mOverlays.rend(); iter++) {
		if ((*iter))
			return ((*iter)->SetKeyBoardFocus(inFocus));
	}
