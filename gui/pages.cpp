...
int PageSet::Load(ZipArchive* package)
{
    ...
    set_scale_values(0, 0); // Reset any previous scaling values
	...
	if (child) {
		...
		if (resolution) {
			...
			if (width_attr && height_attr && !noscale_attr) {
				...
				if (width != 0 && height != 0) {
					float scale_w = (float)gr_fb_width() / (float)width;
					float scale_h = (float)gr_fb_height() / (float)height;
					LOGINFO("Scaling theme width %fx and height %fx\n", scale_w, scale_h);
