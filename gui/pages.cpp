...
void PageManager::Translate_Partition(const char* path, const char* resource_name, const char* default_value) {
	TWPartition* part = PartitionManager.Find_Partition_By_Path(path);
	if (part) {
		part->Display_Name = GetResources()->FindString(resource_name, default_value);
		part->Backup_Display_Name = part->Display_Name;
	}
}

void PageManager::Translate_Partition(const char* path, const char* resource_name, const char* default_value, const char* storage_resource_name, const char* storage_default_value) {
	TWPartition* part = PartitionManager.Find_Partition_By_Path(path);
	if (part) {
		part->Display_Name = GetResources()->FindString(resource_name, default_value);
		part->Backup_Display_Name = part->Display_Name;
		if (part->Is_Storage)
			part->Storage_Name = GetResources()->FindString(storage_resource_name, storage_default_value);
	}
}

void PageManager::Translate_Partition_Display_Names() {
	if (!GetResources()) {
		LOGERR("No resources available for Translate_Partition_Display_Names\n");
		return;
	}
	...
	if (part)
		part->Backup_Display_Name = GetResources()->FindString("android_secure", "Android Secure");
