...
std::string LoadAttrString(xml_node<>* element, const char* attrname, const char* defaultvalue, int depth /* = 0 */)
{
	if (!element)
		return defaultvalue;
	...
	xml_node<>* style = element->first_node("style");
	if (style) {
		while (style) {
			if (!style->first_attribute("name")) {
				LOGERR("No name given for style.\n");
				...
				if (node) {
					// We found the style that was named
					std::string stylevalue = LoadAttrString(node, attrname, defaultvalue, depth + 1);
					if (stylevalue != defaultvalue)
						return stylevalue;
				}
			}
			style = style->next_sibling("style");
		...
		// Search for stylename in the parent node <object type="foo" stylename="foo2">
		attr = element->first_attribute("style");
		// If no style is found anywhere else and the element wasn't found in the object itself
		// as a special case we will search for a style that uses the same style name as the
		// object type, so <object type="button"> would search for a style named "button"
		if (!attr)
			attr = element->first_attribute("type");
		if (attr) {
			xml_node<>* node = PageManager::FindStyle(attr->value());
			if (node) {
				std::string stylevalue = LoadAttrString(node, attrname, defaultvalue);
				if (stylevalue != defaultvalue)
					return stylevalue;
			}
		}
