...
int PageSet::Load(ZipArchive* package)
{
	...
	return CheckAdditional(package, &mDoc);
}

int PageSet::CheckAdditional(ZipArchive* package, xml_document<> *parentDoc)
{
	xml_node<>* par;
	xml_node<>* par2;
	xml_node<>* chld;
	...
	while (chld != NULL)
	{
		...
		// Now, let's parse the XML
		LOGINFO("Loading additional resources...\n");
		child = parent->first_node("resources");
		if (child)
			mResources->LoadResources(child, package);

		LOGINFO("Loading additional variables...\n");
		child = parent->first_node("variables");
		if (child)
			LoadVariables(child);
		...
		LOGINFO("Loading additional pages...\n");
		// This may be NULL if no templates are present
		xmltemplate = parent->first_node("templates");
		if (xmltemplate)
			templates.push_back(xmltemplate);
		...
		if (CheckAdditional(package, &doc))
			return -1;
