...
int GUIPartitionList::GetListSize(void)
{
	return mList.size();
}

int GUIPartitionList::GetListItem(int item_index, std::string &text)
{
	if (item_index < 0 || item_index >= mList.size()) {
		icon = NULL;
		text = "";
		return -1;
	} else {
...
void GUIPartitionList::NotifySelect(int item_selected)
{
	if (item_selected >= 0 && item_selected < mList.size()) {
		int listSize = mList.size();
		if (ListType == "mount") {
		...
		LOGINFO("Invalid item selected %i in GUIPartitionList::NotifySelect\n", item_selected);
	mUpdate = 1;
}
