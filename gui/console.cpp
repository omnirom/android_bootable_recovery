...
GUIConsole::GUIConsole(xml_node<>* node) : GUIScrollList(node)
{
	...
	else
	{
		...
		if (child)
		{
			COLOR ForegroundColor = LoadAttrColor(child, "foreground", COLOR(255,255,255,255));
			mBackgroundColor = LoadAttrColor(child, "background", mBackgroundColor);
			//mScrollColor = LoadAttrColor(child, "scroll", mScrollColor);
			mFontColor = mFontHighlightColor = ForegroundColor;
		}
...
int GUIConsole::RenderConsole(void)
{
	...
	// TODO: fix condition - the current one also fires for partially visible last line
	if (firstDisplayedItem + GetDisplayItemCount() + 1 >= (int)rConsole.size())
		scrollToEnd = true;
#if 1
...
int GUIConsole::NotifyTouch(TOUCH_STATE state, int x, int y)
{
	...
	if (mSlideout && state == TOUCH_START && x >= mSlideoutX && x < mSlideoutX + mSlideoutW && y >= mSlideoutY && y < mSlideoutY + mSlideoutH) {
		if (mSlideoutState == hidden)
