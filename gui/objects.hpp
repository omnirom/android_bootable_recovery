...
protected:
	...
	// get number of items
	virtual size_t GetListSize() { return 0; }
	...
	// Called by the derived class to set the max icon size so we can calculate the proper actualLineHeight and center smaller icons if the icon size varies
...
protected:
	...
	int actualLineHeight; // Actual line height of each line including max icon size, font size, and padding
	int mStart; // this line goes at the top of the display list
	int startY; // used to store the last processed touch location in the y axis - used for debounce (not processed) and for scrolling
	int mSeparatorH, mHeaderSeparatorH; // Height of the separator between lines and the header separator height which may be different
	int mLineSpacing; // stores the spacing or padding on the y axis, part of the actualLineHeight
	...
	int mFastScrollRectX; // x location of the fastscroll rectangle, also set to -1 if no fastscroll bar is present
	...
	int scrollingY; // this is how many pixels offset in the y axis for per pixel scrolling, is always <= 0 and should never be > abs(actualLineHeight)
	int mFontHeight; // height of the font
	...
	COLOR mSeparatorColor; // color of the separator that is between lines
	...
	bool isHighlighted; // indicates that the user has pressed and held on an item so we should highlight the selected row
	...
	int startSelection; // y location of the initial touch, set to -1 if we are scrolling, used to make selections in the list
	int touchDebounce; // debounce for touches, minimum of 6 pixels but may be larger calculated based actualLineHeight / 3
