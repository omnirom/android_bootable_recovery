...
public:
	...
	virtual int GetListSize(void) { return 0; }
	virtual int GetListItem(int item_index, std::string &text)
		{ icon = NULL; text = ""; return -1; }
	virtual void NotifySelect(int item_selected) {}

protected:
	virtual int GetSelection(int x, int y);
	void SetMaxIconSize(int w, int h);
	void SetVisibleListLocation(int list_index);
...
protected:
	...
	int mIconWidth, mIconHeight, mHeaderIconHeight, mHeaderIconWidth;
	int scrollingSpeed;
	int scrollingY;
...
public:
	...
	int GetListItem(int item_index, std::string &text);
	void NotifySelect(int item_selected);
