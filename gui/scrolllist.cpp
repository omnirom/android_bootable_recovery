...
GUIScrollList::GUIScrollList(xml_node<>* node) : GUIObject(node)
{
	...
	mIconWidth = mIconHeight =  mHeaderIconHeight = mHeaderIconWidth = 0;
	...
	itemChosen = mFastScrollRectX = mFastScrollRectY = -1;
...
void GUIScrollList::SetMaxIconSize(int w, int h)
{
	...
	if (h > mIconHeight)
		mIconHeight = h;
	if (mIconHeight > mFontHeight)
		actualLineHeight = mIconHeight + mLineSpacing + mSeparatorH;
}

void GUIScrollList::SetVisibleListLocation(int list_index)
{
	// This will make sure that the item indicated by list_index is visible on the screen
	int lines = mRenderH / (mLineHeight + mLineSpacing), listSize = mList.size();

	if (list_index > mStart + lines - 1)
		mStart = list_index;
	if (mStart > listSize - lines) {
		mStart = listSize - lines + 1;
	} else if (list_index < mStart) {
		mStart = list_index;
	}
...
int GUIScrollList::Render(void)
{
	...
	for (line = 0; line < lines; line++)
	{
		...
		if (mList.at(line + mStart).displayResource)
		{
			icon = mList.at(line + mStart).displayResource;
			if (icon && icon->GetResource()) {
				...
				currentIconOffsetX = (mIconWidth - currentIconWidth) / 2;
	...
	case TOUCH_RELEASE:
		...
		if (startSelection >= 0)
		{
			...
			if (actualSelection < listSize)
			{
				...
				itemChosen = mList.at(actualSelection).list_index;
				mUpdate = 1;
