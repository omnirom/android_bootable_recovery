...
GUIScrollList::GUIScrollList(xml_node<>* node) : GUIObject(node)
{
	...
	firstDisplayedLine = mLineSpacing = mFontHeight = mSeparatorH = y_offset = scrollingSpeed = 0;
...
int GUIScrollList::Render(void)
{
	...
	size_t actualSelection = firstDisplayedLine;
	...
	{
		size_t itemindex = line + firstDisplayedLine;
	...
	if (hasScroll) {
		...
		if ((mRenderH - mHeaderH) % actualLineHeight != 0) {
			...
			pct = ((firstDisplayedLine*actualLineHeight - y_offset)*100)/(listSize*actualLineHeight-((lines + 1)*actualLineHeight) + partial_line_size);
		} else {
			pct = ((firstDisplayedLine*actualLineHeight - y_offset)*100)/(listSize*actualLineHeight-lines*actualLineHeight);
		}
...
int GUIScrollList::NotifyTouch(TOUCH_STATE state, int x, int y)
{
	...
	case TOUCH_DRAG:
		...
		if(fastScroll)
		{
			...
			{
				firstDisplayedLine = totalSize - lines;
				if ((mRenderH - mHeaderH) % actualLineHeight != 0) {
			...
			{
				firstDisplayedLine = l;
				y_offset = -(l - int(l))*actualLineHeight;
				if ((mRenderH - mHeaderH) % actualLineHeight != 0) {
					// There's a partial row displayed, make sure y_offset doesn't go past the max
					if (firstDisplayedLine == totalSize - lines - 1 && y_offset < ((mRenderH - mHeaderH) % actualLineHeight) - actualLineHeight)
						y_offset = ((mRenderH - mHeaderH) % actualLineHeight) - actualLineHeight;
				} else if (firstDisplayedLine == totalSize - lines)
					y_offset = 0;
			}
...
void GUIScrollList::HandleScrolling()
{
	...
	while(firstDisplayedLine && y_offset > 0) {
		firstDisplayedLine--;
		y_offset -= actualLineHeight;
	}
	if (firstDisplayedLine == 0 && y_offset > 0)
	...
	// the offset should always be <= 0 and > -actualLineHeight, adjust the first display row and offset as needed
	while (firstDisplayedLine + lines + (bottom_offset ? 1 : 0) < totalSize && abs(y_offset) > actualLineHeight) {
		firstDisplayedLine++;
		y_offset += actualLineHeight;
	}
	// Check if we dragged too far, set the list at the bottom and adjust offset as needed
	if (bottom_offset != 0 && firstDisplayedLine + lines + 1 >= totalSize && y_offset <= bottom_offset) {
		firstDisplayedLine = totalSize - lines - 1;
		y_offset = bottom_offset;
	} else if (firstDisplayedLine + lines >= totalSize && y_offset < 0) {
		firstDisplayedLine = totalSize - lines;
		y_offset = 0;
	}
}
