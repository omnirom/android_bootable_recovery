...
const int SCROLLING_SPEED_DECREMENT = 6;
const int SCROLLING_FLOOR = 10;
const int SCROLLING_MULTIPLIER = 6;
...
void GUIScrollList::SetVisibleListLocation(size_t list_index)
{
	...
	size_t lines = (mRenderH - mHeaderH) / (actualItemHeight), listSize = GetItemCount();
...
int GUIScrollList::Render(void)
{
	...
	size_t lines = (mRenderH - mHeaderH) / actualItemHeight;
	...
	// render fast scroll
	lines = (mRenderH - mHeaderH) / (actualItemHeight);
	if (hasScroll) {
		...
		if ((mRenderH - mHeaderH) % actualItemHeight != 0) {
			...
			int partial_line_size = actualItemHeight - ((mRenderH - mHeaderH) % actualItemHeight);
...
int GUIScrollList::Update(void)
{
	...
	if (scrollingSpeed == 0) {
		...
		if (scrollingSpeed < ((int) (actualItemHeight * 2.5))) {
			y_offset += scrollingSpeed;
			scrollingSpeed -= SCROLLING_SPEED_DECREMENT;
		} else {
			y_offset += ((int) (actualItemHeight * 2.5));
			scrollingSpeed -= SCROLLING_SPEED_DECREMENT;
		}
		HandleScrolling();
		mUpdate = 1;
	} else if (scrollingSpeed < 0) {
		if (abs(scrollingSpeed) < ((int) (actualItemHeight * 2.5))) {
			y_offset += scrollingSpeed;
			scrollingSpeed += SCROLLING_SPEED_DECREMENT;
		} else {
			y_offset -= ((int) (actualItemHeight * 2.5));
...
int GUIScrollList::NotifyTouch(TOUCH_STATE state, int x, int y)
{
	...
	case TOUCH_DRAG:
		if (fastScroll)
		{
			...
			int lines = (mRenderH - mHeaderH) / actualItemHeight;
			...
			else
			{
				...
				if ((mRenderH - mHeaderH) % actualItemHeight != 0) {
					...
					if (firstDisplayedItem == totalSize - lines - 1 && y_offset < ((mRenderH - mHeaderH) % actualItemHeight) - actualItemHeight)
						y_offset = ((mRenderH - mHeaderH) % actualItemHeight) - actualItemHeight;
...
void GUIScrollList::HandleScrolling()
{
	...
	int totalSize = GetItemCount();
	int lines = (mRenderH - mHeaderH) / (actualItemHeight); // number of full lines our list can display at once
	int bottom_offset = ((int)(mRenderH) - mHeaderH) - (lines * actualItemHeight); // extra display area that can display a partial line for per pixel scrolling

	bottom_offset -= actualItemHeight;
