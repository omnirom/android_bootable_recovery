...
int tw_get_defaults(const char* filename) {
...
int tw_set_defaults(const char* filename) {
	int ret = 0;
	...
	if (lsetfilecon(filename, selinux_context) < 0) {
		//printf("Failed to set default contexts on '%s'.\n", filename);
		ret = -1;
	}

	if (s.st_mode & S_IFREG && chmod(filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH) < 0) {
		//printf("Failed to chmod '%s'\n", filename);
		ret = -1;
	}

	if (chown(filename, s.st_uid, s.st_gid) < 0) {
