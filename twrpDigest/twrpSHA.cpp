...
std::string twrpSHA256::return_sha_digest_string(void) {
	...
	fd = open(twrpSHA256::digest_filename.c_str(), O_RDONLY);
	if (fd < 0) {
		return "";
	}
	...
	while ((bytes = read(fd, &buf, sizeof(buf))) == sizeof(buf)) {
		twrpSHA256::update_stream((unsigned char*)buf, bytes);
	}
...
std::string twrpSHA512::return_sha_digest_string(void) {
	...
	fd = open(twrpSHA512::digest_filename.c_str(), O_RDONLY);
	if (fd < 0) {
		return "";
	}
	...
	while ((bytes = read(fd, &buf, sizeof(buf))) == sizeof(buf)) {
		twrpSHA512::update_stream((unsigned char*)buf, bytes);
	}
