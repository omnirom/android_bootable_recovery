...
public:
	...
	twrpMD5(std::string digest_filename);                                 // Initialize with a filename to compute MD5 upon
	void init();                                                          // Initialize MD5 structures
	bool update(const unsigned char* stream, size_t len);                 // Update MD5 stream with data. Return false if failure to update MD5.
	std::string return_digest_string();                                   // Return MD5 digest as string to callee
	void finalize();                                                      // Finalize and compute MD5

protected:
	std::string digest_filename;                                          // Stores digest file across the class
