...
enum TW_DIGEST {
	DIGEST_MATCH_FAIL = -1, // -1: digest did not match
	DIGEST_OK,              //  0: digest matches
};
...
public:
	...
	bool read_file(std::string digest_filename);                     // Read file into digest algorithm. Return false if unable to read file.
	...
	TW_DIGEST verify_digest(std::string digest);                     // Verify the digest of the file against the passed string
