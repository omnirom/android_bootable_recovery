...
char BatteryLed::getChargingStatus() {
	char cap_s[2];
...
int BatteryLed::setChargingStatus() {
...
#ifdef HTC_LEGACY_LED
int BatteryLed::setHtcChargingStatus() {
...
int BatteryLed::startBatteryCheckLoop(void) {
	int set_status = 0;
	LOGINFO("BatteryLED thread started with loop delay: %d sec\n", checkDelay);
	...
	LOGERR("BatteryLED thread canceled; delay of %d sec not in range %d-%d sec\n",
