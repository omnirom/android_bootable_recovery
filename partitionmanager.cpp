...
int TWPartitionManager::Partition_SDCard(void) {
	...
	string Command, Device, fat_str, ext_str, start_loc, end_loc, ext_format, sd_path, tmpdevice;
	int ext, swap, total_size = 0, fat_size;

	gui_msg("start_partition_sd=Partitioning SD Card...");

	// Locate and validate device to partition
	TWPartition* SDCard = Find_Partition_By_Path("/external_sd");
	if (!SDCard)
		SDCard = Find_Partition_By_Path("/external_sdcard");
	if (!SDCard)
		SDCard = Find_Partition_By_Path("/sdcard");
	...
	if (swap > 0) {
		Command = "mkswap " + format_device + "3";
		TWFunc::Exec_Cmd(Command);
	}
