...
bool TWPartitionManager::Make_Digest(PartitionSettings *part_settings)
{
	...
	if (TWFunc::Path_Exists(Full_File)) {
		digest->setfn(Full_File);
		if (digest->computeDigest() == 0) {
			if (digest->write_digest() == 0) {
		...
		while (index < 1000) {
			...
			if (TWFunc::Path_Exists(filename)) {
				if (digest->computeDigest() == 0) {
					...
					gui_err("digest_compute_error= * Error computing Digest.");
					return false;
				}
			}
			index++;
	...
	if (!Restore_List.empty()) {
		...
		while (end_pos != string::npos && start_pos < Restore_List.size()) {
			...
			if (part_settings.Part != NULL) {
				...
				if (check_digest > 0 && !part_settings.Part->Check_Digest(Restore_Name))
					return false;
				...
				if (part_settings.Part->Has_SubPartition) {
					...
					for (subpart = Partitions.begin(); subpart != Partitions.end(); subpart++) {
						if ((*subpart)->Is_SubPartition && (*subpart)->SubPartition_Of == part_settings.Part->Mount_Point) {
							if (check_digest > 0 && !(*subpart)->Check_Digest(Restore_Name))
								return false;
...
void TWPartitionManager::Set_Restore_Files(string Restore_Name) {
	LOGINFO("TWPartitionManager::Set_Restore_Files\n");
