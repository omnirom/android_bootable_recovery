...
bool TWPartitionManager::Write_Digest(string Full_File) {
	...
	if (use_sha2) {
		...
		if (!TWFunc::stream_file_to_digest(Full_File, digest)) {
			delete digest;
			return false;
		}
		...
		LOGINFO("SHA2 Digest: %s  %s\n", digest_str.c_str(), TWFunc::Get_Filename(Full_File).c_str());
	}
	else  {
		digest = new twrpMD5();
	...
	digest_str = digest_str + "  " + TWFunc::Get_Filename(Full_File) + "\n";
