	...
	if (ListType == "mount") {
		...
		if (DataManager::GetIntValue("tw_has_repack_tools") != 0 && DataManager::GetIntValue("tw_has_boot_slots") != 0) {
			...
			if (boot) {
				...
				struct PartitionList repack_kernel;
...
bool TWPartitionManager::Flash_Image(string& path, string& filename) {
	...
	if (Flash_List == "/repack_ramdisk;") {
		DataManager::SetValue("tw_repack_kernel", "0");
		repack = true;
	} else if (Flash_List == "/repack_kernel;") {
		DataManager::SetValue("tw_repack_kernel", "1");
		repack = true;
	}
	if (repack) {
		...
		gui_msg(Msg("unpacking_image=Unpacking {1}...")(full_filename));
		if (!PartitionManager.Prepare_Repack(full_filename, REPACK_NEW_DIR, true))
			return false;
