...
pthread_mutex_t TWPartitionManager::m_lock_stop_backup = PTHREAD_RECURSIVE_MUTEX_INITIALIZER;
...
int TWPartitionManager::Run_Backup(void) {
	...
	start_pos = 0;
	end_pos = Backup_List.find(";", start_pos);
	while (end_pos != string::npos && start_pos < Backup_List.size() && stop_backup == 0) {
		backup_path = Backup_List.substr(start_pos, end_pos - start_pos);
		backup_part = Find_Partition_By_Path(backup_path);
		if (backup_part != NULL) {
