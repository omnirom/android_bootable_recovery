...
bool TWPartitionManager::Make_Digest(PartitionSettings *part_settings)
{
	...
	if (TWFunc::Path_Exists(Full_File)) {
		if (use_sha2) {
			digest = new twrpSHA256(Full_File);
			digest_filename = Full_File + ".sha2";
		}
		else  {
			digest = new twrpMD5(Full_File);
		...
		while (index < 1000) {
			...
			twrpDigest *split_digest;
			string digest_str;
			if (TWFunc::Path_Exists(digest_src)) {
				if (use_sha2) {
					split_digest = new twrpSHA256(digest_src);
					digest_filename = digest_src + ".sha2";
				}
				else {
					split_digest = new twrpMD5(digest_src);
					digest_filename = digest_src + ".md5";
				}
				digest_str = split_digest->return_digest_string();
				digest_str = digest_str + "  " + TWFunc::Get_Filename(digest_src) + "\n";
