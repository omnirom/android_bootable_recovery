...
bool TWPartitionManager::Backup_Partition(PartitionSettings *part_settings) {
	...
	if (part_settings->Part->Backup(part_settings)) {
		...
		if (part_settings->Part->Has_SubPartition) {
			...
			for (subpart = Partitions.begin(); subpart != Partitions.end(); subpart++) {
				if ((*subpart)->Can_Be_Backed_Up && (*subpart)->Is_SubPartition && (*subpart)->SubPartition_Of == part_settings->Part->Mount_Point) {
					if (!(*subpart)->Backup(part_settings)) {
...
bool TWPartitionManager::Flash_Image(PartitionSettings *part_settings) {
	...
	gui_msg(Msg("img_to_flash=Image to flash: '{1}'")(part_settings->Backup_FileName));

	if (!TWFunc::Path_Exists(part_settings->Backup_FileName)) {
		if (!Mount_By_Path(part_settings->Backup_FileName, true)) {
			return false;
		}
		if (!TWFunc::Path_Exists(part_settings->Backup_FileName)) {
			gui_msg(Msg(msg::kError, "unable_to_locate=Unable to locate {1}.")(part_settings->Backup_FileName));
			return false;
		}
	}
