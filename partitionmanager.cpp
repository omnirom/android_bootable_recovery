...
bool TWPartitionManager::Backup_Partition(TWPartition* Part, const string& Backup_Folder, bool generate_md5, unsigned long *img_time, unsigned long *file_time, ProgressTracking *pt) {
	time_t start, stop;
	int use_compression;
	...
	if (Part->Backup(Backup_Folder, tar_fork_pid, pt)) {
		...
		if (Part->Has_SubPartition) {
			...
			for (subpart = Partitions.begin(); subpart != Partitions.end(); subpart++) {
				if ((*subpart)->Can_Be_Backed_Up && (*subpart)->Is_SubPartition && (*subpart)->SubPartition_Of == Part->Mount_Point) {
					if (!(*subpart)->Backup(Backup_Folder, tar_fork_pid, pt)) {
...
int TWPartitionManager::Run_Backup(void) {
	...
	ProgressTracking pt(total_bytes);
	...
	while (end_pos != string::npos && start_pos < Backup_List.size()) {
		...
		backup_part = Find_Partition_By_Path(backup_path);
		if (backup_part != NULL) {
			if (!Backup_Partition(backup_part, Full_Backup_Path, do_md5, &img_time, &file_time, &pt))
				return false;
		} else {
			gui_msg(Msg(msg::kError, "unable_to_locate_partition=Unable to locate '{1}' partition for backup calculations.")(backup_path));
		}
...
bool TWPartitionManager::Restore_Partition(TWPartition* Part, const string& Restore_Name, ProgressTracking *pt) {
	time_t Start, Stop;
	TWFunc::SetPerformanceMode(true);
	time(&Start);

	if (!Part->Restore(Restore_Name, pt)) {
		TWFunc::SetPerformanceMode(false);
		return false;
	}
	if (Part->Has_SubPartition) {
		...
		for (subpart = Partitions.begin(); subpart != Partitions.end(); subpart++) {
			if ((*subpart)->Is_SubPartition && (*subpart)->SubPartition_Of == Part->Mount_Point) {
				if (!(*subpart)->Restore(Restore_Name, pt)) {
					TWFunc::SetPerformanceMode(false);
					return false;
				}
			}
...
int TWPartitionManager::Run_Restore(const string& Restore_Name) {
	...
	DataManager::SetProgress(0.0);
	ProgressTracking pt(total_restore_size);
	...
	if (!Restore_List.empty()) {
		...
		while (end_pos != string::npos && start_pos < Restore_List.size()) {
			...
			if (restore_part != NULL) {
				partition_count++;
				if (!Restore_Partition(restore_part, Restore_Name, &pt))
					return false;
			} else {
				gui_msg(Msg(msg::kError, "restore_unable_locate=Unable to locate '{1}' partition for restoring.")(restore_path));
			}
