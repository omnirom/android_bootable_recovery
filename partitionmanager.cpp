...
bool TWPartitionManager::Write_ADB_Stream_Header(uint64_t partition_count) {
...
bool TWPartitionManager::Write_ADB_Stream_Trailer() {
		int adb_control_bu_fd;
...
int TWPartitionManager::Run_Backup(bool adbbackup) {
	...
	if (adbbackup) {
		if (Write_ADB_Stream_Header(partition_count) == false) {
			return false;
		}
	}
	...
	if (adbbackup) {
		if (Write_ADB_Stream_Trailer() == false) {
			return false;
		}
	}
