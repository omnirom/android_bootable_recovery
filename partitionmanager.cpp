...
void TWPartitionManager::Update_System_Details(void) {
	...
	/*int ret = 0;
	int fd = open("/dev/block/mmcblk1", O_RDONLY);
	ret = ioctl(fd, BLKRRPART, 0);
	close(fd);
	LOGINFO("ioctl returned %i\n", ret);*/
			...
			if ((*adopt)->Decrypt_Adopted() == 0) {
				if (volumes) {
					...
					while (volume) {
						...
						if (guid) {
							char part_guid[80];
							strcpy(part_guid, (*adopt)->Adopted_GUID.c_str());
							char* ptr = part_guid;
							while (*ptr) {
								*ptr = toupper(*ptr);
								ptr++;
							}
							string GUID = part_guid;
							GUID.insert(8, "-");
							GUID.insert(13, "-");
							GUID.insert(18, "-");
							GUID.insert(23, "-");
							
							if (strcmp(GUID.c_str(), guid->value()) == 0) {
