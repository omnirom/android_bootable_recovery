...
// standalone test: g++ -Wall -DTWMSG_TEST twmsg.cpp && ./a.out
#ifdef TWMSG_TEST
#include <iostream>
#else
#include "data.hpp"
...
class MockResourceLookup : public StringLookup
{
public:
	virtual std::string operator()(const std::string& name) const
	{
		if (name == "backup_complete")
			return "Backup complete.";
		if (name == "backup_size")
			return "Backup size: {tw_partition_backup_size} MB";
		if (name == "unable_to_mount")
			return "Unable to mount {1}: {2}";
		return "[" + name + "]";
	}
};
MockResourceLookup mockResourceLookup;

class MockDataLookup : public StringLookup
{
public:
	virtual std::string operator()(const std::string& name) const
	{
		if (name == "tw_partition_backup_size")
			return "42";
		return "";
	}
};
MockDataLookup mockDataLookup;


Message Msg(const char* name)
{
	return Message(msg::kNormal, name, mockResourceLookup, mockDataLookup);
}

Message Msg(msg::Kind kind, const char* name)
{
	return Message(kind, name, mockResourceLookup, mockDataLookup);
}

int main()
{
	std::string msg;
	msg = Msg("backup_complete");
	std::cout << msg << std::endl;
	msg = Msg("backup_size");
	std::cout << msg << std::endl;
	msg = Msg("unable_to_mount")("system")(strerror(5));
	std::cout << msg << std::endl;
	msg = Msg("unable_to_mount=Can't mount {1} because {2}")("system")(strerror(5));
	std::cout << msg << std::endl;
	msg = Msg("something={2}, sides reversed, is {1}")("cba")("abc");
	std::cout << msg << std::endl;
	msg = Msg("this resource does not exist");
	std::cout << msg << std::endl;
	msg = Msg("={1} files found")(12345);
	std::cout << msg << std::endl;
	return 0;
}
#endif
