...
int OpenRecoveryScript::Backup_ADB_Command(std::string Options) {
	std::vector<std::string> args = TWFunc::Split_String(Options, " ");
	...
	if (args[1].compare("--twrp") != 0) {
		gui_print("--twrp option is required to enable twrp adb backup\n");
		return 1;
	}
			...
			if (cmdstr.substr(0, sizeof(cmdstruct.type) - 1) == TWCNT) {
				...
				if (cmdstr.substr(0, sizeof(TWIMG) - 1) == TWIMG) {
					...
					if (path.compare("/system") == 0) {
						if (part_settings.Part->Is_System_Read_Only()) {
							struct twcmd twerror;
							strncpy(twerror.start_of_header, TWRP, sizeof(twerror.start_of_header));
							strncpy(twerror.type, TWERROR, sizeof(twerror.type));
							memset(twerror.space, 0, sizeof(twerror.space));
