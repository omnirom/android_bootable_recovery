...
int OpenRecoveryScript::check_for_script_file(void) {
	...
	LOGINFO("orsFile: %s\n", orsFile.c_str());
	if (TWFunc::get_cache_dir() == NON_AB_CACHE_DIR) {
		if (!PartitionManager.Mount_By_Path(TWFunc::get_cache_dir(), false)) {
			LOGINFO("Unable to mount %s for OpenRecoveryScript support.\n", TWFunc::get_cache_dir().c_str());
			gui_msg(Msg(msg::kError, "unable_to_mount=Unable to mount {1}")(TWFunc::get_cache_dir().c_str()));
			return 0;
		}
	}
