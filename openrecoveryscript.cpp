		...
		while (fgets(script_line, SCRIPT_COMMAND_SIZE, fp) != NULL && ret_val == 0) {
			...
			if (strcmp(command, "install") == 0) {
				...
				if (ret_val == 1) {
					...
					twerror.crc = crc32(twerror.crc, (const unsigned char*) twerror.start_of_header, sizeof(twerror.start_of_header));
					twerror.crc = crc32(twerror.crc, (const unsigned char*) twerror.type, sizeof(twerror.type));
					twerror.crc = crc32(twerror.crc, (const unsigned char*) twerror.space, sizeof(twerror.space));
					if (write(adb_control_bu_fd, &twerror, sizeof(twerror)) < 0) {
							gui_err("adbbackup_control_error=Cannot write to adb control channel");
						ret_val = 1;
					}
...
int OpenRecoveryScript::Backup_ADB_Command(std::string Options) {
	...
	for (unsigned i = 2; i < args.size(); i++) {
		...
		if (part) {
			...
			gui_msg(Msg(msg::kError, "partition_not_found=path: {1} not found in partititon list")(path));
			return 1;
		}
	}
...
// this is called by the "AdbBackupControlType" GUI action when a command is received via FIFO from the "twrp" command line tool
		...
		if (read(adb_control_twrp_fd, cmd, sizeof(cmd)) > 0) {
			...
			if (cmdstr.substr(0, sizeof(cmdstruct.type) - 1) == TWCNT) {
				...
				if (cmdstr.substr(0, sizeof(TWIMG) - 1) == TWIMG) {
					...
					part_settings.adb_compression = twimghdr.compressed;
					part_settings.Backup_FileName = Backup_FileName;
					LOGINFO("part_settings.Backup_FileName: %s\n", part_settings.Backup_FileName.c_str());
					LOGINFO("part_settings.Restore_Name: %s\n", part_settings.Restore_Name.c_str());
					if (!PartitionManager.Restore_Partition(&part_settings)) {
						LOGERR("ADB Restore failed.\n");
						return -1;
					}
