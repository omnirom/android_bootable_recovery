...
void TWPartition::Setup_Data_Media() {
	...
	if (Mount_Point == "/data") {
		...
		string testing = Mount_Point + "/media/0";
		if (Mount(false) && TWFunc::Path_Exists(Mount_Point + "/media/0")) {
			Storage_Path = Mount_Point + "/media/0";
			Symlink_Path = Storage_Path;
			UnMount(true);
	...
	if (Is_Adopted_Storage) {
		...
		if (ret == 0) {
			...
			if (!Mount(false)) {
				...
				PartitionManager.Add_MTP_Storage(MTP_Storage_ID);
				char* xmlFile = NULL;
				xmlFile = PageManager::LoadFileToBuffer("/data/system/storage.xml", NULL);
				if (xmlFile != NULL) {
					LOGINFO("successfully loaded storage.xml\n");
					xml_document<> *doc = new xml_document<>();
					doc->parse<0>(xmlFile);
					xml_node<>* volumes = doc->first_node("volumes");
					if (volumes) {
						xml_node<>* volume = volumes->first_node("volume");
						while (volume) {
							xml_attribute<>* attr = volume->first_attribute("nickname");
							if (attr) {
								Storage_Name = attr->value();
								Display_Name = Storage_Name;
								Backup_Display_Name = Storage_Name;
