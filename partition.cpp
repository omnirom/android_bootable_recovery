...
/* The following dual format flags can be written as 'flagname' or 'flagname=',
 * where the character following '=' is Y,y,1 for true, and false otherwise:
 * backup, canencryptbackup, userdataencryptbackup, flashimg, storage
 */
		...
		for (; tw_flag->name; tw_flag++) {
			...
			if (strncmp(ptr, tw_flag->name, flag_len) == 0) {
				...
				case TWFLAG_CANENCRYPTBACKUP:
					Can_Encrypt_Backup = flag_val;
					break;
				case TWFLAG_USERDATAENCRYPTBACKUP:
					Use_Userdata_Encryption = flag_val;
				...
				case TWFLAG_FLASHIMG:
					Can_Flash_Img = flag_val;
					break;
				default:
					// Should not get here
					break;
				}
				// User flag processed; can break out of tw_flag->name loop
				// and process next user provided flag
				break;
			}
		}
		if (tw_flag->name == 0) {
