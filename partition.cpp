...
bool TWPartition::Check_MD5(PartitionSettings *part_settings) {
	...
	if (!TWFunc::Path_Exists(Full_Filename)) {
		...
		while (index < 1000) {
			...
			case MD5_OK:
				gui_msg(Msg("md5_matched=MD5 matched for '{1}'.")(split_filename));
				break;
		...
		switch (md5sum.verify_md5digest()) {
		case MD5_OK:
			gui_msg(Msg("md5_matched=MD5 matched for '{1}'.")(Full_Filename));
			break;
		case MD5_FILE_UNREADABLE:
		case MD5_NOT_FOUND:
			gui_msg(Msg(msg::kError, "no_md5_found=No md5 file found for '{1}'. Please unselect Enable MD5 verification to restore.")(Full_Filename));
			return false;
		case MD5_MATCH_FAIL:
			gui_msg(Msg(msg::kError, "md5_fail_match=MD5 failed to match on '{1}'.")(Full_Filename));
			return false;
		}
	}
	return true;
