...
bool TWPartition::Repair() {
	...
	if (Current_File_System == "vfat") {
		if (!TWFunc::Path_Exists("/sbin/dosfsck")) {
			gui_msg(Msg(msg::kError, "repair_not_exist={1} does not exist! Cannot repair!")("dosfsck"));
			return false;
		}
		if (!UnMount(true))
			return false;
		gui_msg(Msg("reparing=Repairing {1} using {2}...")(Display_Name)("dosfsck"));
		Find_Actual_Block_Device();
...
bool TWPartition::Wipe_FAT() {
	...
	if (TWFunc::Path_Exists("/sbin/mkdosfs")) {
		...
		gui_msg(Msg("formating_using=Formatting {1} using {2}...")(Display_Name)("mkdosfs"));
		Find_Actual_Block_Device();
