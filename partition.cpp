...
void TWPartition::Process_FS_Flags(const char *str) {
	...
	for (ptr = strtok_r(options, ",", &savep); ptr; ptr = strtok_r(NULL, ",", &savep)) {
		...
		if (equals && diff <= 0)
			continue; // The equals sign shouldn't be the first char or before the first char

		// There are some flags that we want to ignore in TWRP
		const char** ignored_mount_item = ignored_mount_items;
		for (; *ignored_mount_item; ignored_mount_item++) {
			if (equals) {
				if (strncmp(ptr, *ignored_mount_item, diff) == 0) {
					continue;
				}
			} else {
				if (strcmp(ptr, *ignored_mount_item) == 0) {
					continue;
				}
			}
		}

		// mount_flags are never postfixed by '=', so this must be a mount option that we aren't ignoring
		if (equals) {
