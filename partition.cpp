...
void TWPartition::Recreate_Media_Folder(void) {
	...
	if (!Mount(true)) {
		...
		// We must force tw_fixperms_restorecon to 1 to fix contexts
		// using fix permissions. Afterwards, we will try to set the
		// default metadata that we were hopefully able to get during
		// early boot.
		DataManager::SetValue("tw_fixperms_restorecon", 1);
		perms.fixDataInternalContexts();
		DataManager::SetValue("tw_fixperms_restorecon", 0);
