...
bool TWPartition::Backup(PartitionSettings *part_settings) {
	if (Backup_Method == BM_FILES)
		return Backup_Tar(part_settings);
...
bool TWPartition::Backup_Image(PartitionSettings *part_settings) {
	...
	if (part_settings->adbbackup) {
		...
		strncpy(twimghdr.name, adb_file_name.c_str(), sizeof(twimghdr.name));
...
bool TWPartition::Raw_Read_Write(PartitionSettings *part_settings) {
	...
	if (part_settings->PM_Method == PM_BACKUP) {
		...
		if (part_settings->adbbackup)
			...
			destfn = part_settings->Full_Backup_Path + Backup_FileName;
	}
	else {
		...
		if (part_settings->adbbackup)
			...
			srcfn = part_settings->Restore_Name + "/" + Backup_FileName;
	}
...
bool TWPartition::Flash_Image(PartitionSettings *part_settings) {
	...
	if (Backup_Method == BM_FILES) {
		...
		unsigned long long image_size = TWFunc::Get_File_Size(part_settings->Backup_FileName);
		if (image_size > Size) {
		...
		if (Backup_Method == BM_DD) {
			if (!part_settings->adbbackup) {
				if (Is_Sparse_Image(part_settings->Backup_FileName)) {
					return Flash_Sparse_Image(part_settings->Backup_FileName, part_settings->adbbackup);
				}
			}
			unsigned long long file_size = (unsigned long long)(TWFunc::Get_File_Size(part_settings->Backup_FileName));
			return Raw_Read_Write(part_settings);
		} else if (Backup_Method == BM_FLASH_UTILS) {
			return Flash_Image_FI(part_settings->Backup_FileName, NULL, part_settings->adbbackup);
		}
	}
