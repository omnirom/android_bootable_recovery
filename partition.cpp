...
bool TWPartition::Check_Restore_File_Digest(const string& Filename) {
	...
	string digest_str, digest_check;
	bool use_sha2 = false;

	digest_check = Filename + ".sha2";
	if (TWFunc::Path_Exists(digest_check)) {
		digestfile += ".sha2";
	...
	digest_check = digest->return_digest_string();
	LOGINFO("digest_check: %s\n", digest_check.c_str());
