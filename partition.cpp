...
bool TWPartition::Backup(const string& backup_folder, pid_t &tar_fork_pid, ProgressTracking *pt) {
	if (Backup_Method == FILES) {
		return Backup_Tar(backup_folder, pt, tar_fork_pid);
	}
	else if (Backup_Method == DD)
		return Backup_Image(backup_folder, pt);
...
bool TWPartition::Restore(const string& restore_folder, ProgressTracking *pt) {
	...
	if (Is_File_System(Restore_File_System))
		return Restore_Tar(restore_folder, Restore_File_System, pt);
	else if (Is_Image(Restore_File_System)) {
		return Restore_Image(restore_folder, Restore_File_System, pt);
	}
...
bool TWPartition::Backup_Tar(const string& backup_folder, ProgressTracking *pt, pid_t &tar_fork_pid) {
	char back_name[255];
	...
	if (tar.createTarFork(pt, tar_fork_pid) != 0)
		return false;
	return true;
}

bool TWPartition::Backup_Image(const string& backup_folder, ProgressTracking *pt) {
	string Full_FileName;
	...
	if (!Raw_Read_Write(Actual_Block_Device, Full_FileName, &Backup_Size, pt))
		return false;
...
bool TWPartition::Raw_Read_Write(const string& input_file, const string& output_file, unsigned long long *input_size, ProgressTracking *pt) {
	...
	if (pt)
		pt->SetPartitionSize(*input_size);
	while (Remain > 0) {
...
bool TWPartition::Backup_Dump_Image(const string& backup_folder) {
	char back_name[255];
...
bool TWPartition::Restore_Tar(const string& restore_folder, const string& Restore_File_System, ProgressTracking *pt) {
	string Full_FileName;
	bool ret = false;
	...
	if (!Password.empty())
		tar.setpassword(Password);
#endif
	pt->SetPartitionSize(Get_Restore_Size(restore_folder));
	if (tar.extractTarFork(pt) != 0)
		ret = false;
	else
		ret = true;
#ifdef HAVE_CAPABILITIES
...
bool TWPartition::Restore_Image(const string& restore_folder, const string& Restore_File_System, ProgressTracking *pt) {
	string Full_FileName;
	...
	if (Restore_File_System == "emmc") {
		...
		if (!Raw_Read_Write(Full_FileName, Actual_Block_Device, &file_size, pt))
			return false;
...
bool TWPartition::Flash_Image_FI(const string& Filename) {
	string Command;
