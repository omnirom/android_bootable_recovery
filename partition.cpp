...
bool TWPartition::Check_Restore_File_Digest(const string& Filename) {
	...
	if (use_sha2) {
		//check for SHA2 digest file
		digestfile += ".sha2";
		if (!TWFunc::Path_Exists(digestfile)) {
			//check for md5 digest file
			LOGINFO("Disabling SHA2 digest.\n");
			digest = new twrpMD5(file_name);
			digestfile = Filename + ".md5";
		}
		else {
			//we found a SHA2 digest file
			digest = new twrpSHA256(file_name);
		}
	}
	else {
		//Use MD5 by default
		...
		if (!TWFunc::Path_Exists(digestfile)) {
			//No MD5 digest file found. Try SHA2.
			LOGINFO("MD5 digest not found. Checking for SHA2 digest\n");
			digestfile = Filename + ".sha2";
			if (TWFunc::Path_Exists(digestfile)) {
				digest = new twrpSHA256(file_name);
				use_sha2 = true;
			}
