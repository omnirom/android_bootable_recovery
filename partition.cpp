...
void TWPartition::Setup_Backup(bool Display_Error) {
	...
	if (!Force_Backup_Type)
		Backup_Type = Current_File_System;

	if (Backup_Type == "emmc")
		Backup_Method = DD;
	else if (Backup_Type == "mtd" || Backup_Type == "bml")
		Backup_Method = FLASH_UTILS;
	else if (Is_File_System(Backup_Type))
		Backup_Method = FILES;
	else {
		if (Display_Error)
			LOGERR("Unhandled backup type '%s' on image '%s'\n", Backup_Type.c_str(), Backup_Name.c_str());
		else
			LOGINFO("Unhandled backup type '%s' on image '%s'\n", Backup_Type.c_str(), Backup_Name.c_str());
	}

	if (Is_Image(Backup_Type)) {
...
bool TWPartition::Backup_Tar(string backup_folder, const unsigned long long *overall_size, const unsigned long long *other_backups_size, pid_t &tar_fork_pid) {
	...
	sprintf(back_name, "%s.%s.win", Backup_Name.c_str(), Backup_Type.c_str());
