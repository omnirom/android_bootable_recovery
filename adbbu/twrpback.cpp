	...
	while (breakloop == false) {
		if (read(adb_control_bu_fd, &cmd, sizeof(cmd)) > 0) {
			...
			*/
				...
				AdbBackupFileTrailer imgmd5;

				memset(&imgmd5, 0, sizeof(imgmd5));
				...
				strncpy(imgmd5.md5, md5string.c_str(), sizeof(imgmd5.md5));
				adblogwrite(imgmd5.md5);
				adblogwrite("\n");
				adblogwrite(md5string);
