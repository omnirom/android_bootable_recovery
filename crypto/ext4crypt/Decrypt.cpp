 ...
 * the keystore in this instance. */
bool Find_Keystore_Alias_SubID(const userid_t user_id, std::string& keystoreid) {
...
std::string unwrapSyntheticPasswordBlob(const std::string& spblob_path, const std::string& handle_str, const userid_t user_id, const void* application_id, const size_t application_id_size) {
	...
	if (!Find_Keystore_Alias_SubID(user_id, keystore_alias_subid)) {
		printf("failed to scan keystore alias subid, using handle instead\n");
		keystore_alias_subid = handle_str;
	}
