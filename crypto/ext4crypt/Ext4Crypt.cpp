...
bool Decrypt_User(const userid_t user_id, const std::string& Password) {
    ...
    if (user_id == 0) {
		filename = "/data/system/gatekeeper.password.key";
		...
		filename = "/data/system/users/";
		filename += user_id_str;
		filename += "/gatekeeper.password.key";
	...
	if (stat(filename.c_str(), &st) != 0) {
		printf("error stat'ing key file\n");
		return false;
	}
	std::string handle;
