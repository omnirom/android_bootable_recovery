                            ...
                            unsigned char *ikey, void *params)
{
    ...
    if (keymaster_sign_object_old(ftr, ikey, INTERMEDIATE_BUF_SIZE,
                              &signature, &signature_size)) {
        SLOGE("Signing failed using old keymaster");
        if (keymaster_sign_object(ftr, ikey, INTERMEDIATE_BUF_SIZE,
                              &signature, &signature_size)) {
            SLOGE("Signing failed using new keymaster");
            return -1;
        }
    }
...
SLOGE("cryptfs_check_passwd_hw starting\n");
    /* get key */
    if (get_crypt_ftr_and_key(&crypt_ftr)) {
        SLOGE("Error getting crypt footer and key");
        return -1;
