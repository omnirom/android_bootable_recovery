...
twrpAdbFifo::twrpAdbFifo(void) {
	unlink(TW_ADB_FIFO);
}

bool twrpAdbFifo::Check_Adb_Fifo_For_Events(void) {
	char cmd[512];

	memset(&cmd, 0, sizeof(cmd));

	if (read(adb_fifo_fd, &cmd, sizeof(cmd)) > 0) {
		std::string check_cmd(cmd);
		if (strlen(cmd) != 0)
			LOGINFO("cmd: %s\n", cmd);
...
pthread_t twrpAdbFifo::threadAdbFifo(void) {
	pthread_t thread;
	ThreadPtr adbfifo = &twrpAdbFifo::start;
	PThreadPtr p = *(PThreadPtr*)&adbfifo;
	pthread_create(&thread, NULL, p, this);
	return thread;
}

bool twrpAdbFifo::Backup_ADB_Command(std::string Options) {
...
bool twrpAdbFifo::Restore_ADB_Backup(void) {
