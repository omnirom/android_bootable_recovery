...
struct BackupSettings {                                                        // Settings for backup session   
	...
	uint64_t overall_size;                                                     // Total size of all partitions
	uint64_t other_backups_size;                                               // Keeps remaining total of all partition size for progress indicator
...
struct BackupPartitionSettings {                                               // Partition settings for partition to backup
	TWPartition* Part;                                                         // Partition to pass to partition.cpp
...
struct RestorePartitionSettings {                                              // Parittion settings for partition to restore
	TWPartition* Part;                                                         // Partition generated by partitionmanager
	...
	uint64_t already_restored_size;                                            // keeps track of what has been restored for progress indicator
	bool adbbackup;                                                            // tells the system we are backing up over adb
	int adb_compression;                                                       // 0 == uncompressed, 1 == compressed
	std::string Restore_File_System;                                           // filesystem to format block device
...
private:
	...
	void Get_Restore_File_System(RestorePartitionSettings *part_settings);  // Returns the file system that was in place at the time of the backup
