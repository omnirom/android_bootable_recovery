...
struct BackupSettings {                                                        	   // Settings for backup session   
...
struct BackupPartitionSettings {                                                   // Partition settings for partition to backup
	TWPartition* Part;                                                         // Partition to pass to the partition backup loop 
	std::string Backup_Folder;                                                 // Backup folder to put backup into
	...
	bool adbbackup;                                                            // tell the system we are backing up over adb
	int adb_compression;                                                       // stores the compression mode 
	ProgressTracking *progress;                                                // address for progress tacking class
};

struct RestorePartitionSettings {                                                  // Parittion settings for partition to restore
	TWPartition* Part;                                                         // Partition to pass to the partition backup loop 
	std::string Restore_Name;                                                  // Path to restore folder
	int partition_count;                                                       // Number of partitions to restore
	bool adbbackup;                                                            // tells the system we are backing up over adb
	int adb_compression;                                                       // 0 == uncompressed, 1 == compressed
...
public:
	...
	bool Backup(BackupSettings *backup_settings);                             // Backs up the partition to the folder specified
	bool Check_MD5(string restore_folder);                                    // Checks MD5 of a backup
	bool Restore(RestorePartitionSettings *part_settings); 				  	  // Restores the partition using the backup folder provided
	unsigned long long Get_Restore_Size(RestorePartitionSettings *part_settings);// Returns the overall restore size of the backup
	...
	bool Flash_Image(const string& Filename, bool adbbackup);                                        // Flashes an image to the partition
...
private:
	...
	bool Backup_Tar(BackupSettings *part_settings);                           // Backs up using tar for file systems
	bool Backup_Image(const string& backup_folder, ProgressTracking *progress, bool adbbackup); // Backs up using raw read/write for emmc memory types
	...
	bool Backup_Dump_Image(const string& backup_folder, ProgressTracking *progress, bool adbbackup); // Backs up using dump_image for MTD memory types
	string Get_Restore_File_System(RestorePartitionSettings *part_settings);  // Returns the file system that was in place at the time of the backup
	bool Restore_Tar(RestorePartitionSettings *part_settings);                // Restore using tar for file systems
	bool Restore_Image(RestorePartitionSettings *part_settings);              // Restore using dd for images
	...
	bool Flash_Sparse_Image(const string& Filename, bool adbbackup);          // Flashes a sparse image using simg2img
	bool Flash_Image_FI(const string& Filename, ProgressTracking *progress, bool adbbackup);  // Flashes an image to the partition using flash_image for mtd nand
...
public:
	...
	bool Flash_Image(string Filename, bool adbbackup);                        // Flashes an image to a selected partition from the partition list
	bool Restore_Partition(struct RestorePartitionSettings *part_settings);
...
private:
	...
	bool Backup_Partition(BackupPartitionSettings *part_settings);            // Backup the partitions based on type
	bool Restore_Partition(TWPartition* Part, const string& Restore_Name, ProgressTracking *progress); // Restore the partitions based on type
