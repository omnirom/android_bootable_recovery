...
public:
	...
	bool Backup(const string& backup_folder, pid_t &tar_fork_pid, ProgressTracking *pt); // Backs up the partition to the folder specified
	bool Check_MD5(string restore_folder);                                    // Checks MD5 of a backup
	bool Restore(const string& restore_folder, ProgressTracking *pt);         // Restores the partition using the backup folder provided
...
private:
	...
	bool Backup_Tar(const string& backup_folder, ProgressTracking *pt, pid_t &tar_fork_pid); // Backs up using tar for file systems
	bool Backup_Image(const string& backup_folder, ProgressTracking *pt);     // Backs up using raw read/write for emmc memory types
	bool Raw_Read_Write(const string& input_file, const string& output_file, unsigned long long *input_size, ProgressTracking *pt);
	bool Backup_Dump_Image(const string& backup_folder);                      // Backs up using dump_image for MTD memory types
	...
	bool Restore_Tar(const string& restore_folder, const string& Restore_File_System, ProgressTracking *pt); // Restore using tar for file systems
	bool Restore_Image(const string& restore_folder, const string& Restore_File_System, ProgressTracking *pt); // Restore using raw read/write for images
	...
	bool Flash_Image_FI(const string& Filename);                              // Flashes an image to the partition using flash_image for mtd nand
...
private:
	...
	bool Backup_Partition(TWPartition* Part, const string& Backup_Folder, bool generate_md5, unsigned long *img_time, unsigned long *file_time, ProgressTracking *pt);
	bool Restore_Partition(TWPartition* Part, const string& Restore_Name, ProgressTracking *pt);
