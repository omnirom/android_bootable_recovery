	...
	if (pid == 0) {
		...
		if (use_encryption || userdata_encryption) {
			...
			while ((de = readdir(d)) != NULL) {
				FolderName = tardir + "/" + de->d_name;

				if (de->d_type == DT_BLK || de->d_type == DT_CHR || du.check_skip_dirs(FolderName))
					continue;
				if (de->d_type == DT_DIR) {
					...
					if (userdata_encryption && ((item_len >= 3 && strncmp(de->d_name, "app", 3) == 0) || (item_len >= 6 && strncmp(de->d_name, "dalvik", 6) == 0))) {
						if (Generate_TarList(FolderName, &RegularList, &target_size, &regular_thread_id) < 0) {
							LOGERR("Error in Generate_TarList with regular list!\n");
							closedir(d);
							_exit(-1);
						}
						regular_size += du.Get_Folder_Size(FolderName);
					} else {
						encrypt_size += du.Get_Folder_Size(FolderName);
					}
			...
			while ((de = readdir(d)) != NULL) {
				...
				if (de->d_type == DT_DIR) {
					...
					if (userdata_encryption && ((item_len >= 3 && strncmp(de->d_name, "app", 3) == 0) || (item_len >= 6 && strncmp(de->d_name, "dalvik", 6) == 0))) {
						...
						FolderName = tardir + "/" + de->d_name;
						if (Generate_TarList(FolderName, &EncryptList, &target_size, &enc_thread_id) < 0) {
...
int twrpTar::Generate_TarList(string Path, std::vector<TarListStruct> *TarList, unsigned long long *Target_Size, unsigned *thread_id) {
	...
	while ((de = readdir(d)) != NULL) {
		...
		if (de->d_type == DT_DIR) {
			...
			if (Generate_TarList(FolderName, TarList, Target_Size, thread_id) < 0)
				return -1;
		} else if (de->d_type == DT_REG || de->d_type == DT_LNK) {
			stat(FolderName.c_str(), &st);
			TarList->push_back(TarItem);
