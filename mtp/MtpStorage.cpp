		...
		mServer(refserver)
{
	...
	inotify_thread = 0;
	inotify_fd = -1;
	// Threading has not started yet so we should be save to set these directly instead of using atomics
	inotify_thread_kill = 0;
	inotify_thread_dead = 0;
	sendEvents = false;
...
MtpStorage::~MtpStorage() {
	...
	if (use_mutex) {
		use_mutex = false;
		while (__tw_atomic_cmpxchg(0, inotify_thread_dead, &inotify_thread_dead) == 0)
...
int MtpStorage::inotify_t(void) {
	...
	while (__tw_atomic_cmpxchg(0, inotify_thread_kill, &inotify_thread_kill) == 0) {
		...
		while (i < len && __tw_atomic_cmpxchg(0, inotify_thread_kill, &inotify_thread_kill) == 0) {
			...
			if (event->len) {
				...
				lockMutex(1);
				if (__tw_atomic_cmpxchg(0, inotify_thread_kill, &inotify_thread_kill) == 0)
	...
	close(inotify_fd);*/
	__tw_atomic_cmpxchg(0, 1, &inotify_thread_dead);
	//inotify_thread_dead = 1;
	return 0;
}
